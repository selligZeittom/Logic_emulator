...
void InputView::initGraphicalObject()
{
    ...
    load->setStyleSheet("QPushButton { background-color : green; border: 1px solid gray; border-radius: 5px; color : white; }");
    load->setText("Browse a Json file");
    load->show();

    connect(load, SIGNAL(clicked(bool)), this, SLOT(buttonClicked()));
}
